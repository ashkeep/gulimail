...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
    ...
    @Override
    public void spuBigSaveAll(SpuAllSaveVo spuInfo) throws Exception {
...
//        int i = 10/0;
     ...
     */
    @Override
    public void updateSpuStatus(String spuId, Integer status) {
        if (status == 1) {
            //上架
            spuUp(spuId, status);
        } else {
     ...
     * @param status
     */
    private void spuUp(String spuId, Integer status) {
        //1、查出我们接下来要使用的基本信息；
...
//        List<ProductAttrValueEntity> productAttrValueEntities = new ArrayList<>();
        ...
        list.forEach((item)->{
            ...
            spu_id.forEach((s)->{
                if(item.getAttrId() == s.getAttrId()){
                    ...
                    value.setSpuId(Long.valueOf(spuId));
        ...
        if (skus != null && skus.size() > 0) {
            ...
            if (resp.getCode() == 0) {
                ...
                entity.setId(Long.valueOf(spuId));
